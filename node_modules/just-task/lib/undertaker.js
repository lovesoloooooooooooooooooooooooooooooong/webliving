...
const undertaker_1 = __importDefault(require("undertaker"));
const logger_1 = require("./logger");
const chalk_1 = __importDefault(require("chalk"));
const wrapTask_1 = require("./wrapTask");
const cache_1 = require("./cache");
const undertaker = new undertaker_1.default();
exports.undertaker = undertaker;
const NS_PER_SEC = 1e9;
let topLevelTask = undefined;
let errorReported = false;
const tasksInProgress = {};
const colors = [chalk_1.default.cyanBright, chalk_1.default.magentaBright, chalk_1.default.blueBright, chalk_1.default.greenBright, chalk_1.default.yellowBright];
const taskColor = {};
let colorIndex = 0;
...
undertaker.on('stop', function (args) {
    if (shouldLog(args)) {
        ...
        logger_1.logger.info(`finished '${colorizeTaskName(args.name)}' in ${chalk_1.default.yellow(String(durationInSecs) + 's')}`);
    }
});
undertaker.on('error', function (args) {
    ...
    if (!errorReported) {
        errorReported = true;
        logger_1.logger.error(chalk_1.default.red(`Error detected while running '${colorizeTaskName(args.name)}'`));
        logger_1.logger.error(chalk_1.default.yellow('------------------------------------'));
        const stackOrMessage = args.error.stack || args.error.message || args.error;
        if (stackOrMessage) {
            logger_1.logger.error(chalk_1.default.yellow(stackOrMessage));
        }
        if (args.error.stdout) {
            logger_1.logger.error(chalk_1.default.yellow('stdout:'));
            logger_1.logger.error(args.error.stdout);
        }
        if (args.error.stderr) {
            logger_1.logger.error(chalk_1.default.yellow('stderr:'));
        ...
        const duration = args.duration;
        const durationInSecs = Math.round(((duration[0] * NS_PER_SEC + duration[1]) / NS_PER_SEC) * 100) / 100;
        logger_1.logger.error(`finished '${colorizeTaskName(args.name)}' in ${chalk_1.default.yellow(String(durationInSecs) + 's')} with ${chalk_1.default.red('errors')}`);
        process.exitCode = 1;
    }
...
process.on('exit', code => {
    if (code !== 0) {
        logger_1.logger.error(chalk_1.default.dim(`Error previously detected. See above for error messages.`));
    }
